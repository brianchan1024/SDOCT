...
Background::~Background()
{
	...
	if (d_bg_noise) gpuErrchk( cudaFree(d_bg_noise) );
}
...
{
	// load bg frame into array
	float2 *bg_frame = new float2[height_1fr * width]();  // filled with zeroes
	data.loadFile(p.fnameBg, 0, (height_1fr*width), bg_frame);					
	
	// get mean of each depth in a-scan. This is d_bg.
	float *bg_column_mean = new float[width];
	bgColumnMeanMax = 0;
	Helper::columnMean(height_1fr, width, bg_frame, bg_column_mean, bgColumnMeanMax);

	gpuErrchk( cudaMalloc((void **)&d_bg, (p.batchAscans * width * sizeof(float))) );
	gpuErrchk( cudaMemcpy(d_bg, bg_column_mean, (width * sizeof(float)), cudaMemcpyHostToDevice) );

	repmat<<<dimLine_w,dimLine_B>>>(p.batchAscans, width, d_bg); gpuErrchk( cudaPeekAtLastError() );

	// bg frame and maximum mean are used in the next function
	gpuErrchk( cudaMalloc((void **)&d_bg_frame, (height_1fr * width * sizeof(float2))) );
	gpuErrchk( cudaMemcpy(d_bg_frame, bg_frame, (height_1fr * width * sizeof(float2)), cudaMemcpyHostToDevice) );

	delete[] bg_frame;
	delete[] bg_column_mean;

	cout << "	- Generated bg matrix" << endl;
}
...
void Background::genBgMask()
{
	...
	float2 *d_bg_temp;
	gpuErrchk( cudaMalloc((void **)&d_bg_temp, (width * sizeof(float2))) );
	gpuErrchk( cudaMemset(d_bg_temp, 0, (width * sizeof(float2))) );

	divide<<<dimLine_w,dimLine_B>>>(1, width, d_bg, bgColumnMeanMax, d_bg_temp); gpuErrchk( cudaPeekAtLastError() );
	...
	FFT(1, width, d_bg_temp, d_bg_temp);
	zero_pad<<<dimGrid_w,dimGrid_B>>>(1, width, d_bg_temp, d_bg_temp_zp); gpuErrchk( cudaPeekAtLastError() );
	...
	interp.procInterp(1, width_2x, dimLine_w2, dimLine_B, d_bg_temp_zp, d_bg_mask);
	...
	cout << "	- Generated bg mask matrix" << endl;
}

void Background::genBgNoise()
{
	// follow matlab script if you need to understand this code.
	subtract<<<dimLine_w,dimLine_B>>>(height_1fr, width, d_bg_frame, d_bg, d_bg_frame); gpuErrchk( cudaPeekAtLastError() );

	FFT(height_1fr, width, d_bg_frame, d_bg_frame);

	float2 *d_bg_frame_zp;
	...
	zero_pad<<<dimGrid_w,dimGrid_B>>>(height_1fr, width, d_bg_frame, d_bg_frame_zp); gpuErrchk( cudaPeekAtLastError() );
	...
	float *bg_noise = new float[height_1fr * (width - 10)];
	for (int i = 0; i < height_1fr ; i++)
		for (int j = 10; j < width; j++)
			bg_noise[i*(width-10)+j-10] = bg_noise_temp[i*width_2x+j];
	delete[] bg_noise_temp;

	float *noise_lvl = new float[width - 10];
	columnMean(height_1fr, (width - 10), bg_noise, noise_lvl);
	delete[] bg_noise;

	vector<float> noise_lvl_vec(width - 10);
	for (int i=0; i < (width - 10); i++)
		noise_lvl_vec[i] = noise_lvl[i];
	delete[] noise_lvl;

	float *noise_lvl_res = new float[width - 10];
	vector<float> filterA(10, 0.1f);
	vector<float> a(1, 1.0f);

	filtfilt(9, a, filterA, (width - 10), noise_lvl_vec, noise_lvl_res);
	...
	cout << "	- Generated bg noise matrix" << endl;
}
...
void Background::filter(int ord, vector<float> &a, vector<float> &b, int w, vector<float> &my_vector, vector<float> &result_vector)
{
...
void Background::filtfilt(int ord, vector<float> &a, vector<float> &b, int w, vector<float> &my_vector, float *result_array)
{
	int nfilt;
	...
	vector<float> temp_result_vector(IC_vector_width);
	filter(ord, a, b, IC_vector_width, my_vector_IC, temp_result_vector);

	vector<float> temp_data(IC_vector_width);
